
# Docs and Resources for everything

 This file contains documentation/resources links for different topic.

# Contribute  

 Add more docs and resources 

 ## Programming language
 - [Java(oracle)](https://docs.oracle.com/javase/8/)
 - [Java](https://dev.java/learn/)
 - [C](https://en.cppreference.com/w/c/language.html)
 - [C++](https://cppreference.com/)
 - [Python](https://docs.python.org/3/)
 - [Go](https://go.dev/doc/) 
 - [Rust](https://www.rust-lang.org/learn) 
 - [Ruby](https://www.ruby-lang.org/en/documentation/) 
 - [Kotlin](https://kotlinlang.org/docs/home.html#) 
 - [Javascript]() 
 - [Typescipt](https://www.typescriptlang.org/docs/) 
 - [Perl](https://perldoc.perl.org/) 

# 🌐 Web development

Direct links to official documentation for commonly used frontend and backend technologies.

---

## 🔹 Frontend Technologies

### 📄 Markup & Styling
- [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML)
- [CSS](https://developer.mozilla.org/en-US/docs/Web/CSS)
- [SASS / SCSS](https://sass-lang.com/documentation)
- [Tailwind CSS](https://tailwindcss.com/docs)
- [Bootstrap](https://getbootstrap.com/docs/)
- [Styled Components](https://styled-components.com/docs)
- [Material UI](https://mui.com/material-ui/getting-started/overview/)
- [PostCSS](https://postcss.org/docs/)

---

### ⚙️ JavaScript & TypeScript
- [JavaScript (MDN)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
- [TypeScript](https://www.typescriptlang.org/docs/)
- [jQuery](https://api.jquery.com/)

---

### 🧩 Frontend Libraries & Frameworks
- [React.js](https://react.dev/learn)
- [Next.js](https://nextjs.org/docs)
- [Vue.js](https://vuejs.org/guide/introduction.html)
- [Nuxt.js](https://nuxt.com/docs/getting-started/introduction)
- [Angular](https://angular.io/docs)
- [Svelte](https://svelte.dev/docs)
- [Solid.js](https://www.solidjs.com/docs)
- [Qwik](https://qwik.builder.io/docs/overview/)

---

### 🛠 Build Tools
- [Webpack](https://webpack.js.org/concepts/)
- [Vite](https://vitejs.dev/guide/)
- [Parcel](https://parceljs.org/docs/)
- [Rollup](https://rollupjs.org/introduction/)
- [Babel](https://babeljs.io/docs/en/)
- [ESBuild](https://esbuild.github.io/getting-started/)

---

### 🧪 Testing (Frontend)
- [Jest](https://jestjs.io/docs/getting-started)
- [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/)
- [Cypress](https://docs.cypress.io/)
- [Mocha](https://mochajs.org/)
- [Chai](https://www.chaijs.com/guide/)
- [Playwright](https://playwright.dev/docs/intro)

---

### 🌐 Other Frontend Tools
- [Axios](https://axios-http.com/docs/intro)
- [React Query](https://tanstack.com/query/latest/docs/react/overview)
- [SWR](https://swr.vercel.app/docs)
- [Redux](https://redux.js.org/introduction/getting-started)
- [Zustand](https://docs.pmnd.rs/zustand/getting-started/introduction)
- [Recoil](https://recoiljs.org/docs/introduction/installation)
- [Jotai](https://jotai.org/docs/introduction)
- [Framer Motion](https://www.framer.com/motion/introduction/)
- [Three.js](https://threejs.org/docs/index.html)
- [Chart.js](https://www.chartjs.org/docs/latest/)
- [Recharts](https://recharts.org/en-US/)
- [D3.js](https://github.com/d3/d3/wiki)

---

## 🔹 Backend Technologies

### 🔧 Programming Languages
- [Node.js](https://nodejs.org/en/docs)
- [PHP](https://www.php.net/docs.php)
- [C# (.NET)](https://learn.microsoft.com/en-us/dotnet/csharp/)

---

### 🧰 Backend Frameworks
- [Express.js](https://expressjs.com/en/starter/installing.html)
- [NestJS](https://docs.nestjs.com/)
- [FastAPI](https://fastapi.tiangolo.com/)
- [Flask](https://flask.palletsprojects.com/en/latest/)
- [Django](https://docs.djangoproject.com/en/stable/)
- [Spring Boot](https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/)
- [Gin (Go)](https://gin-gonic.com/docs/)
- [Laravel](https://laravel.com/docs)
- [Ruby on Rails](https://guides.rubyonrails.org/)
- [ASP.NET](https://learn.microsoft.com/en-us/aspnet/core/?view=aspnetcore-8.0)

---

### 🗃 Databases

#### SQL:
- [PostgreSQL](https://www.postgresql.org/docs/)
- [MySQL](https://dev.mysql.com/doc/)
- [SQLite](https://www.sqlite.org/docs.html)
- [MS SQL Server](https://learn.microsoft.com/en-us/sql/sql-server/)

#### NoSQL:
- [MongoDB](https://www.mongodb.com/docs/)
- [Redis](https://redis.io/docs/)
- [Cassandra](https://cassandra.apache.org/doc/latest/)
- [Firebase Firestore](https://firebase.google.com/docs/firestore)

---

### 🌐 API Technologies
- [REST](https://restfulapi.net/)
- [GraphQL](https://graphql.org/learn/)
- [gRPC](https://grpc.io/docs/)
- [WebSockets (MDN)](https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API)

---

### 🛠 ORMs & ODMs
- [Prisma](https://www.prisma.io/docs/)
- [Sequelize](https://sequelize.org/docs/v6/)
- [TypeORM](https://typeorm.io/)
- [Mongoose](https://mongoosejs.com/docs/)
- [SQLAlchemy](https://docs.sqlalchemy.org/en/20/)
- [Entity Framework](https://learn.microsoft.com/en-us/ef/)

---

### 🧪 Testing (Backend)
- [Jest](https://jestjs.io/docs/getting-started)
- [Supertest](https://www.npmjs.com/package/supertest)
- [Mocha](https://mochajs.org/)
- [Chai](https://www.chaijs.com/guide/)
- [Postman](https://learning.postman.com/docs/)
- [Insomnia](https://docs.insomnia.rest/)
- [Pytest](https://docs.pytest.org/en/stable/)

---

### 🔐 Authentication & Security
- [JWT](https://jwt.io/introduction)
- [OAuth 2.0](https://oauth.net/2/)
- [Passport.js](http://www.passportjs.org/docs/)
- [Bcrypt (Node)](https://www.npmjs.com/package/bcrypt)
- [Argon2](https://github.com/P-H-C/phc-winner-argon2)
- [Auth0](https://auth0.com/docs)
- [Firebase Auth](https://firebase.google.com/docs/auth)
- [CORS (MDN)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)
- [CSRF (OWASP)](https://owasp.org/www-community/attacks/csrf)

---
## ⚙️ DevOps

This document provides direct links to official documentation for key tools and platforms used in modern **DevOps** workflows.

---

## 🏗️ CI/CD (Continuous Integration & Delivery)

- [GitHub Actions](https://docs.github.com/en/actions)
- [GitLab CI/CD](https://docs.gitlab.com/ee/ci/)
- [CircleCI](https://circleci.com/docs/)
- [Jenkins](https://www.jenkins.io/doc/)
- [Travis CI](https://docs.travis-ci.com/)
- [Bitbucket Pipelines](https://bitbucket.org/product/features/pipelines)

---

## 🐳 Containerization

- [Docker](https://docs.docker.com/)
- [Docker Compose](https://docs.docker.com/compose/)
- [Podman](https://docs.podman.io/)

---

## ☸️ Container Orchestration

- [Kubernetes](https://kubernetes.io/docs/)
- [Helm (Kubernetes Package Manager)](https://helm.sh/docs/)
- [Kustomize](https://kubectl.docs.kubernetes.io/references/kustomize/)

---

## ⚙️ Infrastructure as Code (IaC)

- [Terraform](https://developer.hashicorp.com/terraform/docs)
- [Pulumi](https://www.pulumi.com/docs/)
- [AWS CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/Welcome.html)
- [Ansible](https://docs.ansible.com/)
- [Chef](https://docs.chef.io/)
- [SaltStack](https://docs.saltproject.io/)

---

## 🛠️ Configuration Management

- [Ansible](https://docs.ansible.com/)
- [Puppet](https://puppet.com/docs/)
- [Chef](https://docs.chef.io/)

---

## 🌩️ Cloud Platforms

- [Amazon Web Services (AWS)](https://docs.aws.amazon.com/)
- [Microsoft Azure](https://learn.microsoft.com/en-us/azure/)
- [Google Cloud Platform (GCP)](https://cloud.google.com/docs)
- [DigitalOcean](https://docs.digitalocean.com/)
- [Linode](https://www.linode.com/docs/)
- [Heroku](https://devcenter.heroku.com/)
- [Render](https://docs.render.com/)

---

## 🧪 Monitoring & Logging

- [Prometheus](https://prometheus.io/docs/introduction/overview/)
- [Grafana](https://grafana.com/docs/)
- [ELK Stack (Elasticsearch, Logstash, Kibana)](https://www.elastic.co/guide/index.html)
- [Fluentd](https://docs.fluentd.org/)
- [Datadog](https://docs.datadoghq.com/)
- [New Relic](https://docs.newrelic.com/)

---

## 🔐 Secrets & Security

- [HashiCorp Vault](https://developer.hashicorp.com/vault/docs)
- [AWS Secrets Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/intro.html)
- [SOPS (Secrets OPerationS)](https://github.com/getsops/sops)
- [Sealed Secrets (Bitnami)](https://github.com/bitnami-labs/sealed-secrets)
- [Kubernetes Secrets](https://kubernetes.io/docs/concepts/configuration/secret/)

---

## 📦 Artifact Repositories

- [JFrog Artifactory](https://jfrog.com/help/r/jfrog-artifactory-documentation)
- [Nexus Repository](https://help.sonatype.com/repomanager3)
- [GitHub Packages](https://docs.github.com/en/packages)
- [GitLab Package Registry](https://docs.gitlab.com/ee/user/packages/)

---

## 📈 Performance & Load Testing

- [Apache JMeter](https://jmeter.apache.org/usermanual/index.html)
- [k6 (by Grafana)](https://k6.io/docs/)
- [Locust](https://docs.locust.io/en/stable/)
- [Artillery](https://www.artillery.io/docs/)

---

## 📋 Version Control & GitOps

- [Git](https://git-scm.com/doc)
- [GitHub](https://docs.github.com/)
- [GitLab](https://docs.gitlab.com/)
- [ArgoCD (GitOps for Kubernetes)](https://argo-cd.readthedocs.io/en/stable/)
- [FluxCD (GitOps)](https://fluxcd.io/docs/)

---

## 🔄 Reverse Proxy & Load Balancer

- [NGINX](https://docs.nginx.com/)
- [HAProxy](https://www.haproxy.org/documentation.html)
- [Traefik](https://doc.traefik.io/traefik/)

---

## 🧹 Code Quality & Linting

- [SonarQube](https://docs.sonarsource.com/)
- [ESLint](https://eslint.org/docs/latest/)
- [Prettier](https://prettier.io/docs/en/index.html)
- [Pylint](https://pylint.pycqa.org/en/latest/)

---

## 🎯 Misc DevOps Tools

- [Minikube (Local K8s)](https://minikube.sigs.k8s.io/docs/)
- [Kind (Kubernetes IN Docker)](https://kind.sigs.k8s.io/docs/)
- [Vagrant](https://developer.hashicorp.)

# 📱 Android Development 

This document provides direct links to official documentation for tools, languages, frameworks, and libraries used in Android app development.

---

## 🧱 Core Programming Languages

- [Kotlin](https://kotlinlang.org/docs/home.html)
- [Java](https://docs.oracle.com/en/java/)
- [C++ (NDK)](https://developer.android.com/ndk)

---

## ⚙️ Android SDK & Frameworks

- [Android Developer Guide](https://developer.android.com/guide)
- [Android SDK API Reference](https://developer.android.com/reference)
- [Jetpack Libraries](https://developer.android.com/jetpack)
- [Jetpack Compose (UI Toolkit)](https://developer.android.com/jetpack/compose/documentation)

---

## 🖼 UI & Layout

- [Views and ViewGroups](https://developer.android.com/guide/topics/ui/declaring-layout)
- [Material Design Components](https://m3.material.io/develop/android)
- [ConstraintLayout](https://developer.android.com/develop/ui/views/layout/constraint-layout)
- [Jetpack Compose](https://developer.android.com/jetpack/compose/documentation)

---

## 🔀 Navigation & Architecture

- [Navigation Component](https://developer.android.com/guide/navigation)
- [ViewModel](https://developer.android.com/topic/libraries/architecture/viewmodel)
- [LiveData](https://developer.android.com/topic/libraries/architecture/livedata)
- [Room (Database)](https://developer.android.com/jetpack/androidx/releases/room)
- [DataStore](https://developer.android.com/topic/libraries/architecture/datastore)
- [WorkManager](https://developer.android.com/topic/libraries/architecture/workmanager)

---

## 🔗 Networking & APIs

- [Retrofit](https://square.github.io/retrofit/)
- [OkHttp](https://square.github.io/okhttp/)
- [Volley](https://developer.android.com/training/volley)

---

## 📦 Dependency Injection

- [Hilt (by Jetpack)](https://developer.android.com/training/dependency-injection/hilt-android)
- [Dagger](https://dagger.dev/)

---

## 🔐 Authentication & Security

- [Firebase Authentication](https://firebase.google.com/docs/auth/android/start)
- [Biometric Authentication](https://developer.android.com/training/sign-in/biometric-auth)
- [SafetyNet](https://developer.android.com/training/safetynet)
- [Encrypted SharedPreferences](https://developer.android.com/topic/security/data)

---

## 🔎 Testing

- [JUnit (Android)](https://developer.android.com/training/testing/unit-testing/local-unit-tests)
- [Espresso (UI Testing)](https://developer.android.com/training/testing/espresso)
- [Mockito](https://site.mockito.org/)
- [UI Automator](https://developer.android.com/training/testing/ui-automator)

---

## 🔔 Notifications & Background Work

- [Notifications](https://developer.android.com/develop/ui/views/notifications)
- [Foreground Services](https://developer.android.com/guide/components/foreground-services)
- [AlarmManager](https://developer.android.com/reference/android/app/AlarmManager)
- [JobScheduler](https://developer.android.com/reference/android/app/job/JobScheduler)

---

## ☁️ Firebase Integration

- [Firebase for Android](https://firebase.google.com/docs/android/setup)
- [Firebase Firestore](https://firebase.google.com/docs/firestore)
- [Firebase Cloud Messaging](https://firebase.google.com/docs/cloud-messaging)
- [Firebase Crashlytics](https://firebase.google.com/docs/crashlytics)
- [Firebase Analytics](https://firebase.google.com/docs/analytics)

---

## 🧪 Debugging & Performance

- [Logcat](https://developer.android.com/studio/debug/logcat)
- [Android Profiler](https://developer.android.com/studio/profile)
- [LeakCanary](https://square.github.io/leakcanary/)
- [Stetho (by Facebook)](https://github.com/facebook/stetho)

---

## 🚀 Build, CI/CD, & Distribution

- [Gradle](https://developer.android.com/build)
- [Android App Bundles](https://developer.android.com/guide/app-bundle)
- [Firebase App Distribution](https://firebase.google.com/docs/app-distribution)
- [Play Console](https://play.google.com/console/about/)
- [Fastlane](https://docs.fastlane.tools/)

---

## 📚 Additional Resources

- [Android Developer Training](https://developer.android.com/courses)
- [Kotlin Android Docs](https://developer.android.com/kotlin)
- [Compose Pathway](https://developer.android.com/jetpack/compose/tutorial)

---

# 🍎 iOS Development 

This document provides direct links to official documentation for technologies used in iOS app development.

---

## 🧱 Core Programming Languages

- [Swift](https://swift.org/documentation/)
- [Objective-C](https://developer.apple.com/documentation/objectivec)

---

## ⚙️ iOS SDK & Frameworks

- [iOS Developer Guide](https://developer.apple.com/documentation/)
- [UIKit](https://developer.apple.com/documentation/uikit)
- [SwiftUI](https://developer.apple.com/documentation/swiftui)
- [Foundation Framework](https://developer.apple.com/documentation/foundation)
- [Combine (Reactive Framework)](https://developer.apple.com/documentation/combine)

---

## 🖼 UI & Layout

- [SwiftUI Documentation](https://developer.apple.com/documentation/swiftui)
- [UIKit Documentation](https://developer.apple.com/documentation/uikit)
- [Auto Layout](https://developer.apple.com/documentation/uikit/nslayoutconstraint)
- [Interface Builder (Storyboard/XIB)](https://developer.apple.com/xcode/interface-builder/)

---

## 🔄 App Architecture

- [MVVM with SwiftUI](https://developer.apple.com/videos/play/wwdc2020/10040/)
- [Clean Architecture (Uncle Bob)](https://8thlight.com/blog/uncle-bob/2012/08/13/the-clean-architecture.html)
- [State Management with Combine](https://developer.apple.com/documentation/combine)
- [Core Data](https://developer.apple.com/documentation/coredata)

---

## 🔌 Networking

- [URLSession](https://developer.apple.com/documentation/foundation/urlsession)
- [Combine Networking](https://developer.apple.com/documentation/combine)
- [Alamofire](https://alamofire.github.io/Alamofire/)

---

## ☁️ Backend as a Service (BaaS)

- [Firebase for iOS](https://firebase.google.com/docs/ios/setup)
- [Firebase Firestore](https://firebase.google.com/docs/firestore)
- [Firebase Realtime Database](https://firebase.google.com/docs/database)
- [Firebase Authentication](https://firebase.google.com/docs/auth/ios/start)
- [Apple CloudKit](https://developer.apple.com/icloud/cloudkit/)

---

## 🔐 Authentication & Security

- [Sign in with Apple](https://developer.apple.com/sign-in-with-apple/)
- [Face ID & Touch ID (Local Authentication)](https://developer.apple.com/documentation/localauthentication)
- [Keychain Services](https://developer.apple.com/documentation/security/keychain_services)
- [App Transport Security](https://developer.apple.com/documentation/bundleresources/information_property_list/nsapptransportsecurity)

---

## 🧪 Testing

- [XCTest](https://developer.apple.com/documentation/xctest)
- [XCUITest](https://developer.apple.com/documentation/xctest/user_interface_tests)
- [SnapshotTesting](https://github.com/pointfreeco/swift-snapshot-testing)
- [Quick & Nimble](https://github.com/Quick/Quick)

---

## 🧩 Package & Dependency Management

- [Swift Package Manager (SPM)](https://developer.apple.com/documentation/swift_packages)
- [CocoaPods](https://guides.cocoapods.org/)
- [Carthage](https://github.com/Carthage/Carthage)

---

## 🔔 Notifications & Background Work

- [Push Notifications](https://developer.apple.com/documentation/usernotifications)
- [Background Modes](https://developer.apple.com/documentation/foundation/urlsession/handling_background_tasks)
- [UNUserNotificationCenter](https://developer.apple.com/documentation/usernotifications/unusernotificationcenter)

---

## 🧪 Debugging & Profiling

- [Xcode Instruments](https://developer.apple.com/documentation/instruments)
- [LLDB Debugger](https://lldb.llvm.org/)
- [Console & Breakpoints](https://developer.apple.com/documentation/xcode/debugging_tools)

---

## 📦 App Distribution

- [TestFlight](https://developer.apple.com/testflight/)
- [App Store Connect](https://developer.apple.com/app-store-connect/)
- [Xcode Build & Archive](https://developer.apple.com/documentation/xcode/building-your-app)

---

## 🎨 Design Guidelines

- [Human Interface Guidelines (HIG)](https://developer.apple.com/design/human-interface-guidelines)
- [SF Symbols](https://developer.apple.com/sf-symbols/)

---

## 📚 Additional Resources

- [iOS Developer Documentation](https://developer.apple.com/documentation/)
- [Swift Language Guide](https://docs.swift.org/swift-book/LanguageGuide/TheBasics.html)
- [SwiftUI Tutorials](https://developer.apple.com/tutorial)

---
# ⛓️ Blockchain Development Tech Stack

A curated list of essential technologies and official documentation links for blockchain developers.

---

## 💻 Core Programming Languages

- [Solidity (Ethereum Smart Contracts)](https://docs.soliditylang.org/)
- [Vyper (Pythonic Smart Contracts)](https://docs.vyperlang.org/)
- [Rust (Solana, NEAR, Polkadot, etc.)](https://www.rust-lang.org/learn)
- [Go (Hyperledger, Tendermint)](https://go.dev/doc/)
- [Move (Aptos, Sui)](https://move-language.github.io/move/)
- [Cairo (StarkNet)](https://book.cairo-lang.org/)

---

## ⚙️ Blockchain Platforms

- [Ethereum Docs](https://ethereum.org/en/developers/docs/)
- [Solana Docs](https://docs.solana.com/)
- [Polygon Docs](https://wiki.polygon.technology/docs/home/)
- [Binance Smart Chain](https://docs.bnbchain.org/)
- [Avalanche Docs](https://docs.avax.network/)
- [Near Protocol Docs](https://docs.near.org/)
- [Polkadot Docs](https://docs.polkadot.network/)
- [StarkNet Docs](https://docs.starknet.io/)
- [Aptos Docs](https://aptos.dev/)
- [Sui Docs](https://docs.sui.io/)

---

## 🔗 Smart Contract Development

- [Solidity Documentation](https://docs.soliditylang.org/)
- [Hardhat (Ethereum Dev Environment)](https://hardhat.org/docs)
- [Truffle Suite](https://trufflesuite.com/docs/)
- [Foundry (Ethereum toolkit)](https://book.getfoundry.sh/)
- [Brownie (Python + Solidity)](https://eth-brownie.readthedocs.io/en/stable/)
- [Remix IDE](https://remix.ethereum.org/)
- [Ganache (Personal Blockchain)](https://trufflesuite.com/ganache/)

---

## 🧪 Testing & Debugging

- [Hardhat Testing](https://hardhat.org/guides/test-contracts.html)
- [Foundry Forge Tests](https://book.getfoundry.sh/reference/forge/)
- [Brownie Testing](https://eth-brownie.readthedocs.io/en/stable/tests.html)

---

## 📦 Package & Dependency Managers

- [npm (Node Package Manager)](https://docs.npmjs.com/)
- [pnpm](https://pnpm.io/)
- [yarn](https://yarnpkg.com/)

---

## 🧾 Web3 Libraries

- [Ethers.js Docs](https://docs.ethers.org/v6/)
- [Web3.js Docs](https://web3js.readthedocs.io/)
- [Viem (TypeScript Web3)](https://viem.sh/docs/introduction.html)
- [Wagmi (React Hooks for Web3)](https://wagmi.sh/docs/getting-started)

---

## 💼 Wallet Integration

- [MetaMask Docs](https://docs.metamask.io/)
- [WalletConnect Docs](https://docs.walletconnect.com/)
- [RainbowKit (with Wagmi)](https://www.rainbowkit.com/docs/introduction)

---

## 🌐 Frontend Frameworks (dApp)

- [Next.js](https://nextjs.org/docs)
- [React.js](https://reactjs.org/docs/getting-started.html)
- [Vue.js](https://vuejs.org/guide/introduction.html)
- [Svelte](https://svelte.dev/docs)

---

## ☁️ Decentralized Storage

- [IPFS Docs](https://docs.ipfs.tech/)
- [Filecoin Docs](https://docs.filecoin.io/)
- [Arweave Docs](https://docs.arweave.org/)
- [Pinata (IPFS Gateway)](https://docs.pinata.cloud/)
- [Web3.Storage](https://web3.storage/docs/)

---

## 🔐 Identity & Authentication

- [Ethereum EIP-4361 (Sign-In with Ethereum)](https://eips.ethereum.org/EIPS/eip-4361)
- [Ceramic Network](https://developers.ceramic.network/)
- [SIWE (Sign-In With Ethereum)](https://docs.login.xyz/)

---

## 🔍 Blockchain Indexing & APIs

- [The Graph Protocol](https://thegraph.com/docs/)
- [Alchemy](https://docs.alchemy.com/)
- [Infura](https://docs.infura.io/)
- [Moralis](https://docs.moralis.io/)

---

## 🧠 Oracles & Interoperability

- [Chainlink Docs](https://docs.chain.link/)
- [Band Protocol](https://docs.bandchain.org/)
- [LayerZero](https://docs.layerzero.network/)

---

## 🔧 DevOps, CI/CD, & Monitoring

- [Tenderly (Monitoring + Debugging)](https://docs.tenderly.co/)
- [Defender (by OpenZeppelin)](https://docs.openzeppelin.com/defender/)
- [Docker](https://docs.docker.com/)
- [GitHub Actions](https://docs.github.com/en/actions)

---

## 📊 Analytics & Dashboards

- [Dune Analytics](https://dune.com/docs/)
- [Nansen](https://www.nansen.ai/)
- [Covalent](https://www.covalenthq.com/docs/api/)

---

## 📚 Additional Learning Resources

- [CryptoZombies](https://cryptozombies.io/)
- [OpenZeppelin Docs (Security Library)](https://docs.openzeppelin.com/)
- [Buildspace](https://buildspace.so/)
- [ChainShot (now Alchemy University)](https://university.alchemy.com/)
- [Ethernaut (by OpenZeppelin)](https://ethernaut.openzeppelin.com/)

---
# 🛡️ Cybersecurity Tech Stack

A curated list of essential tools, technologies, and learning resources for cybersecurity professionals — with official documentation links.

---

## 🧰 Core Tools & Frameworks

- [Kali Linux Docs](https://docs.kali.org/)
- [Parrot OS Docs](https://docs.parrotsec.org/)
- [Metasploit Framework](https://docs.rapid7.com/metasploit/)
- [Burp Suite](https://portswigger.net/burp/documentation)
- [Nmap (Network Mapper)](https://nmap.org/book/man.html)
- [Wireshark (Packet Analysis)](https://www.wireshark.org/docs/)
- [Aircrack-ng (WiFi hacking)](https://www.aircrack-ng.org/documentation.html)
- [Hydra (Password Cracking)](https://github.com/vanhauser-thc/thc-hydra)
- [John the Ripper](https://www.openwall.com/john/doc/)
- [Hashcat (GPU Password Cracker)](https://hashcat.net/wiki/)

---

## ⚔️ Offensive Security (Ethical Hacking / Red Team)

- [Metasploit Docs](https://docs.rapid7.com/metasploit/)
- [Nmap Docs](https://nmap.org/book/man.html)
- [Burp Suite Pro](https://portswigger.net/burp/documentation)
- [SQLMap (SQLi)](https://sqlmap.org/)
- [BeEF (Browser Exploitation)](https://github.com/beefproject/beef/wiki)
- [Empire (Post-exploitation)](https://bc-security.gitbook.io/empire-wiki/)
- [Exploit-DB](https://www.exploit-db.com/)
- [Pwntools (CTF framework)](https://docs.pwntools.com/en/stable/)
- [Impacket (SMB, RDP attacks)](https://pypi.org/project/impacket/)

---

## 🛡️ Defensive Security (Blue Team / SOC)

- [Suricata (IDS/IPS)](https://docs.suricata.io/)
- [Snort IDS](https://snort.org/documents)
- [Zeek (formerly Bro)](https://docs.zeek.org/)
- [OSSEC (Host-based IDS)](https://www.ossec.net/docs/)
- [Wazuh (SIEM)](https://documentation.wazuh.com/)
- [Graylog (Log Management)](https://go2docs.graylog.org/)
- [Splunk (SIEM)](https://docs.splunk.com/)
- [ELK Stack (ElasticSearch, Logstash, Kibana)](https://www.elastic.co/guide/en/elastic-stack-get-started/current/get-started-elastic-stack.html)
- [AlienVault OSSIM](https://cybersecurity.att.com/documentation)

---

## 🔐 Cryptography

- [GnuPG (PGP encryption)](https://gnupg.org/documentation/)
- [OpenSSL Docs](https://www.openssl.org/docs/)
- [Hashing Algorithms – NIST](https://csrc.nist.gov/publications/sp)
- [Cryptography in Python](https://cryptography.io/en/latest/)
- [NaCl/libsodium](https://doc.libsodium.org/)

---

## 🌐 Web Security

- [OWASP Top 10](https://owasp.org/www-project-top-ten/)
- [OWASP Cheat Sheet Series](https://cheatsheetseries.owasp.org/)
- [Burp Suite Docs](https://portswigger.net/burp/documentation)
- [ZAP Proxy (OWASP ZAP)](https://www.zaproxy.org/docs/)
- [Nikto Web Scanner](https://cirt.net/Nikto2)
- [Wapiti Scanner](https://wapiti.sourceforge.io/)

---

## 📱 Mobile Security

- [MobSF (Mobile Security Framework)](https://mobsf.github.io/docs/)
- [Frida (Dynamic Instrumentation)](https://frida.re/docs/home/)
- [Drozer (Android Pentesting)](https://github.com/FSecureLABS/drozer)
- [Objection (Runtime Mobile Exploitation)](https://github.com/sensepost/objection)

---

## 🧪 Malware Analysis & Reverse Engineering

- [Ghidra (NSA RE Tool)](https://ghidra-sre.org/)
- [IDA Free (Disassembler)](https://hex-rays.com/ida-free/)
- [Radare2](https://book.rada.re/)
- [x64dbg](https://x64dbg.com/)
- [Cuckoo Sandbox](https://cuckoosandbox.org/)
- [PEStudio](https://www.winitor.com/)

---

## 💻 Binary Exploitation & CTF

- [pwntools](https://docs.pwntools.com/en/stable/)
- [GDB (GNU Debugger)](https://sourceware.org/gdb/current/onlinedocs/gdb/)
- [ROPgadget](https://github.com/JonathanSalwan/ROPgadget)
- [Pwndbg (GDB Enhancer)](https://github.com/pwndbg/pwndbg)

---

## ☁️ Cloud Security

- [AWS Security Hub](https://docs.aws.amazon.com/securityhub/latest/userguide/what-is-securityhub.html)
- [GCP Security Docs](https://cloud.google.com/security)
- [Azure Security Center](https://learn.microsoft.com/en-us/azure/security-center/)
- [ScoutSuite](https://github.com/nccgroup/ScoutSuite)
- [Prowler (AWS Security)](https://github.com/prowler-cloud/prowler)

---

## 👮 Governance, Risk & Compliance (GRC)

- [NIST Cybersecurity Framework](https://www.nist.gov/cyberframework)
- [ISO 27001 Standard](https://www.iso.org/isoiec-27001-information-security.html)
- [CIS Benchmarks](https://www.cisecurity.org/cis-benchmarks/)
- [MITRE ATT&CK Framework](https://attack.mitre.org/)
- [NIST 800 Series Docs](https://csrc.nist.gov/publications/sp800)

---

## 🔎 OSINT & Reconnaissance

- [Maltego Docs](https://docs.maltego.com/)
- [Shodan API Docs](https://developer.shodan.io/)
- [theHarvester](https://github.com/laramies/theHarvester)
- [SpiderFoot](https://www.spiderfoot.net/documentation/)
- [Amass](https://github.com/owasp-amass/amass)
- [Recon-ng](https://github.com/lanmaster53/recon-ng)

---

## 📚 Learning Platforms & Practice Labs

- [Hack The Box](https://help.hackthebox.com/)
- [TryHackMe](https://tryhackme.com/)
- [PentesterLab](https://pentesterlab.com/)
- [PortSwigger Web Academy](https://portswigger.net/web-security)
- [CTFtime](https://ctftime.org/)
- [OverTheWire](https://overthewire.org/wargames/)
- [CyberDefenders (Blue Team Labs)](https://cyberdefenders.org/)

---

## 📦 Package Managers & Repos

- [APT (Debian/Kali)](https://wiki.debian.org/Apt)
- [Pacman (Arch/Parrot)](https://wiki.archlinux.org/title/pacman)
- [pip (Python)](https://pip.pypa.io/en/stable/)
- [Gem (Ruby)](https://guides.rubygems.org/)

---

## 📓 Additional Resources

- [MITRE ATT&CK](https://attack.mitre.org/)
- [Red Team Tools List](https://github.com/yeyintminthuhtut/Awesome-Red-Teaming)
- [Blue Team Tools List](https://github.com/meitar/awesome-cybersecurity-blueteam)

---






 

